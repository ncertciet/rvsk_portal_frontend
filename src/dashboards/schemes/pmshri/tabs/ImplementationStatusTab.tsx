...
export default function ImplementationStatusTab({ selectedState, states }: ImplementationStatusTabProps) {
  ...
  // Fetch state-wise data for tooltip
  useEffect(() => {
    apiClient
      .get('/schemes/PM_SHRI/statewise')
      .then((res) => {
  ...
  const findStateData = (geoName: string): StatewiseData | undefined => {
    ...
    return statewiseData.find((s) => {
      const normalizedState = s.stateName.toLowerCase().replace(/[&]/g, 'and').replace(/\s+/g, ' ').trim();
      return (
    ...
    const mapData = geoData.features
      ?.map((feature: any) => {
        ...
        const isSelected = selectedState
          ? name.toLowerCase().includes(selectedState.toLowerCase()) ||
            selectedState.toLowerCase().includes(name.toLowerCase())
          : false;

        let value = 0;
        if (isImplemented) value = stateData?.totalSchools || 1;
        if (isSelected) value = (stateData?.totalSchools || 1) + 10000; // Highlight selected

        // Filter visibility
        if (!showImplemented && isImplemented && !isSelected) return null;
        if (!showNotImplemented && !isImplemented) return null;

        return {
          name,
          value,
          schoolCount: stateData?.totalSchools || 0,
          isImplemented,
          stateName: stateData?.stateName || name,
        };
      })
      .filter(Boolean) || [];
    ...
    return {
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          if (!data || !data.isImplemented) {
            return `<div style="padding:4px 8px">
              <div style="font-weight:600;margin-bottom:4px">Nation Wise PM SHRI SCHOOLS</div>
          ...
          return `<div style="padding:4px 8px">
            ...
            <div>Total PM SHRI Schools : <strong>${data.schoolCount.toLocaleString('en-IN')}</strong></div>
          </div>`;
        },
      },
      visualMap: {
    ...
    <Box sx={{ position: 'relative' }}>
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 0.5 }}>
          ...
          <Typography variant="caption" sx={{ cursor: 'pointer', color: '#6B7280' }}>⟳</Typography>
        </Box>
        <FormControlLabel
          control={
            <Checkbox
        ...
        >
          <Box sx={{ textAlign: 'center' }}>
            ...
            <Typography variant="body2" color="text.secondary">
              Map visualization showing PM SHRI implementation status across states.
            </Typography>
            <Typography variant="caption" color="text.secondary" sx={{ mt: 1, display: 'block' }}>
              {states.length} states with PM SHRI schools
            </Typography>
          </Box>
        </Box>
      )}
