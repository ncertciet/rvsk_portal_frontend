...
interface DistrictwiseTabProps {
  selectedState: string;
  selectedDistrict: string;
}

const INNER_TABS = [
  'School', 'Physical Infrastructures', 'Digital Infrastructures',
  'Laboratories', 'Teachers', 'Students', 'Enrollment by Streams', 'PTR',
];
...
const CATEGORY_LABELS: Record<string, string> = {
  prePrimary: 'Pre-Primary School', primary: 'Primary School',
  upperPrimary: 'Upper Primary School', secondary: 'Secondary School',
...
const TYPE_LABELS: Record<string, string> = {
  boys: 'Boys School', girls: 'Girls School', coEd: 'Co-ed School',
};
...
function getSchoolChartOption(stats: DetailedStats) {
  ...
  categoryData.forEach(([key, val], i) => {
    allSeries.push({ name: CATEGORY_LABELS[key] || key, data: [Math.round((val / total) * 10000) / 100, 0, 0], color: CHART_COLORS.category[i % CHART_COLORS.category.length] });
  });
  managementData.forEach(([key, val], i) => {
    allSeries.push({ name: MANAGEMENT_LABELS[key] || key, data: [0, Math.round((val / total) * 10000) / 100, 0], color: CHART_COLORS.management[i % CHART_COLORS.management.length] });
  });
  typeData.forEach(([key, val], i) => {
    allSeries.push({ name: TYPE_LABELS[key] || key, data: [0, 0, Math.round((val / total) * 10000) / 100], color: CHART_COLORS.type[i % CHART_COLORS.type.length] });
  });
  ...
  return {
    ...
    series: allSeries.map((s) => ({ name: s.name, type: 'bar' as const, stack: 'total', data: s.data, itemStyle: { color: s.color }, barMaxWidth: 30 })),
  };
}
...
function getGenericBarOption(data: Record<string, number>, title: string) {
  ...
  return {
    ...
    series: entries.map((_, i) => ({ name: labels[i], type: 'bar' as const, stack: 'total', data: [values[i]], itemStyle: { color: colors[i % colors.length] }, barMaxWidth: 30 })),
  };
}
...
function getTeacherBarOption(data: Record<string, number>) {
  ...
  return {
    ...
    series: entries.map(([, v], i) => ({ name: labels[i], type: 'bar' as const, stack: 'total', data: [v], itemStyle: { color: colors[i % colors.length] }, barMaxWidth: 30 })),
  };
}
...
export default function DistrictwiseTab({ selectedState, selectedDistrict }: DistrictwiseTabProps) {
  ...
  useEffect(() => {
    if (!localState) { setStats(null); return; }
    setLoading(true);
    ...
    <Box>
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 2, alignItems: 'center' }}>
        <FormControl size="small" sx={{ minWidth: 200 }}>
          ...
          <Select value={localState} label="State Name" onChange={(e) => { setLocalState(e.target.value); setLocalDistrict(''); }}>
            {states.map((s) => <MenuItem key={s} value={s}>{s}</MenuItem>)}
          </Select>
        </FormControl>
        <FormControl size="small" sx={{ minWidth: 200 }}>
          <InputLabel>District Name</InputLabel>
          <Select value={localDistrict} label="District Name" onChange={(e) => setLocalDistrict(e.target.value)} disabled={!localState}>
